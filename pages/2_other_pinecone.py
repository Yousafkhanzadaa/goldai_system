...
def showChatUi():
  with rightCol:
    st.title("Global Chat With GOLD AI")
  ...
  with leftCol:
    with st.spinner("Indexing documents... this might take a while⏳"):
      for uploaded_file in up_files:
        ...
        if file_name in st.session_state.processed_files_1:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
          ...
          if index_name in pc.list_indexes().names(): 
            ...
            st.session_state.processed_files_1.append(file_name)
          else:
...
def render_header():
  with leftCol:
    st.title('Global GOLD AI Embedding')
...
def upload_files():
  ...
  with leftCol:
    ...
    for file in st.session_state.processed_files_1:
...
if __name__ == '__main__':
  ...
  if "processed_files_1" not in st.session_state:
    st.session_state.processed_files_1 = []
